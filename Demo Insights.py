...
def func(choice):
	...
	if choice == "Insights":
		...
		# 			df_twitter['date']=pd.to_datetime(df_twitter['tweeted_time'])
		# 			#df_twitter.drop('tweeted_time',axis=1,inplace=True)

		# 			df_twitter['year']=pd.DatetimeIndex(df_twitter['date']).year
		# 			df_twitter['month']=pd.DatetimeIndex(df_twitter['date']).month
		# 			df_twitter['year']=df_twitter['year'].astype('object')

		# 			df_copy=df_twitter.copy()
		# 			#df_twitter=df_twitter[df_twitter['year']==2018]
		# 			#forming a separate feature for cleaned tweets
		# 			for i,v in enumerate(df_twitter['Insight'].astype(str)):
		# 			    #print(str(i)+" "+v)
		# 			    df_twitter.loc[i,'Insight_word_cloud'] = prep.clean(v) #remove mentions, urls, hashtags, emoticons
		# 	return df_twitter
			...
			def Message_distribution(data):
				...
				ans_2=data['year'].value_counts().sort_index()
				label=ans_2.index
				val=ans_2.values
				plt.plot(label,val)
			...
			def kic_graph(data):
				...
				final_kic['KIC-1_Pred'].value_counts().plot(kind= 'bar')
				#plt.title('Distribution of Intents', fontsize=25)
				plt.xticks(rotation = 70,fontsize=15)
				plt.yticks(fontsize=15)
				plt.ylabel('Counts', fontsize=15)
				plt.xlabel('KIC', fontsize=15)
				val=final_kic['KIC-1_Pred'].value_counts()
			...
			def specialty(data):
				...
				return plt
			def down(data):
				data.to_csv("datasample.csv",index=False)
			...
			#click = 0
			# with st.empty():
			# 	df = st.file_uploader("Upload Data for Analysis",type=["xlsx"])
			# if df is not None:
			# 	df_twitter = upload(df)
				#st.write("Data:")
				#st.dataframe(df_twitter.head())
				#st.write("")
			#df['tweeted_time'] = df['tweet_date']
			...
			with st.spinner("Analysing the Data"):
				...
				#click  = button1.checkbox("Please click here to view the Analysis")

			#if click:
				#button1.empty()

			if df is not None:
				selection_list = list(data1['HCP Name'].unique())
				...
				if hcpname is not None and hcpname!=" ":
					...
					with st.spinner("Ploting the Graphs"):
						...
						#final_kic.to_csv("finalkyc.csv",index=False,encoding="utf-8")
						st.markdown("""---""")

						# col12, col13 = st.columns(2)
						# #st.dataframe(data)
						# with col12:
						# 	st.markdown('**<p style="font-size:20px;border-radius:2%;text-align:center;">Where are the HCPs from:question:</p>**',unsafe_allow_html=True)
						# 	plt = country(data)
						# 	st.pyplot(plt)

						# with col13:
						# 	st.markdown('**<p style="font-size:20px;border-radius:2%;text-align:center;">What are Professions that HCPs belongs to:question:</p>**',unsafe_allow_html=True)
						# 	plt = specialty(data)
						# 	st.pyplot(plt)					
						# #st.markdown("""---""")
